...
test.describe('Simple Page Test', () => {
  test('should navigate to radar search page (without results)', async ({
    page,
  }) => {
    ...
    await page.waitForSelector('h1, [data-testid], button, input', {
      timeout: 10000,
    })
  ...
  test('should find radar-related elements', async ({ page }) => {
    // Adicionar token de autenticação fake para bypass do middleware
    await page.context().addCookies([
      {
        name: 'token',
