...
function formatRole(role: UserRoleListItem['role']) {
...
export function ChamadosImpersonationBar() {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await getUsersOnlyWithRoles()
  ...
  const ownRoleLabel = useMemo(() => {
    if (!profile?.id) return null
    const ownUser = users.find((u) => u.id === profile.id)
    return ownUser?.role ?? null
  }, [users, profile?.id])
  ...
  if (!config.enableImpersonation) return null
  ...
  const control = (() => {
    ...
    if (users.length === 0) {
      return (
        <span className="text-xs text-muted-foreground">
          Nenhum usuário com perfil atribuído para personificação.
        </span>
      )
    }
      ...
      >
        ...
        <SelectContent position="popper" sideOffset={6}>
          <SelectItem value={IMPERSONATE_NONE_VALUE} className="text-xs">
            {ownRoleLabel ? `Sua visão (${ownRoleLabel})` : 'Sua visão'}
          </SelectItem>
